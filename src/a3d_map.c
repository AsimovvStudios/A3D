...
bool a3d_map_submit(a3d* e, const a3d_map* map, const mat4 view, const mat4 proj)
{
	...
	for (Uint32 i = 0; i < map->entity_count; i++)
	{
		...
		glm_translate(mvp.model, (vec3){entity->position[0], entity->position[1], entity->position[2]});
		/* apply rotations */
		...
		glm_scale(mvp.model, (vec3){entity->scale[0], entity->scale[1], entity->scale[2]});
...
)
{
	...
	entity->material = a3d_assets_create_textured_material(assets, A3D_ASSET_INVALID_HANDLE, texture, build->tint);
	if (entity->material == A3D_ASSET_INVALID_HANDLE)
	{
		if (texture != A3D_ASSET_INVALID_HANDLE)
		...
		a3d_assets_release_mesh(e, assets, entity->mesh);
		entity->mesh = A3D_ASSET_INVALID_HANDLE;
		a3d_map_set_error(err, err_size, "%s:%u failed material create for entity '%s'", path, line_no, entity->name);
		return false;
	}
