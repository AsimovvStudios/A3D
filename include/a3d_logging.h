...
#define A3D_LOG__PRINT(stream, LEVELSTR, LEVELCOL, TAG, fmt, ...)                                                      \
	...
	{                                                                                                                  \
		...
		fprintf(                                                                                                       \
		    (stream),                                                                                                  \
		    A3D_LOG__C_TIME "%s" A3D_ANSI_RESET " " LEVELCOL "%-5s" A3D_ANSI_RESET A3D_LOG__TAG_FMT A3D_LOG__SRC_FMT   \
		                    " " fmt "\n",                                                                              \
		    a3d_log_time_hhmmss(_t),                                                                                   \
		    (LEVELSTR),                                                                                                \
		    A3D_LOG__TAG_ARG(TAG) A3D_LOG__SRC_ARGS,                                                                   \
		    ##__VA_ARGS__                                                                                              \
		);                                                                                                             \
	} while (0)
...
/* api */
#define A3D_LOG(fmt, ...) fprintf(stdout, fmt "\n", ##__VA_ARGS__)

#define A3D_LOG_INFO(fmt, ...)                                                                                         \
	A3D_LOG__PRINT(stdout, A3D_LOG__LEVEL_INFO, A3D_LOG__C_INFO, A3D_LOG_TAG, fmt, ##__VA_ARGS__)
#define A3D_LOG_WARN(fmt, ...)                                                                                         \
	A3D_LOG__PRINT(stderr, A3D_LOG__LEVEL_WARN, A3D_LOG__C_WARN, A3D_LOG_TAG, fmt, ##__VA_ARGS__)
#define A3D_LOG_ERROR(fmt, ...)                                                                                        \
	A3D_LOG__PRINT(stderr, A3D_LOG__LEVEL_ERROR, A3D_LOG__C_ERROR, A3D_LOG_TAG, fmt, ##__VA_ARGS__)

#if !defined(NDEBUG)
#define A3D_LOG_DEBUG(fmt, ...)                                                                                        \
	A3D_LOG__PRINT(stdout, A3D_LOG__LEVEL_DEBUG, A3D_LOG__C_DEBUG, A3D_LOG_TAG, fmt, ##__VA_ARGS__)
#else
